...
class GantryLayoutWidget_Sidebar extends GantryLayout {
	...
	function render($params = array()) {
		...
		if (property_exists($rparams, 'start') && $rparams->start == $rparams->widget_id) {
			ob_start();
			?>
		<div class="rt-sidebar-wrapper rt-grid-<?php echo $rparams->widget_map[$rparams->position]['gridCount'];?> <?php echo $rparams->widget_map[$rparams->position]['pushPull']; ?>">
		...
		if (property_exists($rparams, 'end') && $rparams->end == $rparams->widget_id) {
			$params[0]['position_close'] = "</div></div></div>";
		}
