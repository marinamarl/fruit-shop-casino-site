...
class GantryDropdownMenuLayout extends AbstractRokMenuLayout {
	...
	public function stageHeader() {
		...
		if ( !self::$jsLoaded && ( ( $gantry->get( 'layout-mode' ) == 'responsive' ) || ( $gantry->get('layout-mode' ) == 'fluidresponsive' ) ) ) {
	...
	protected function renderItem( RokMenuNode &$item, &$menu ) {
		...
		$dd_large_direction = $item->getAttribute( 'large_direction' );
		$dd_desktop_direction = $item->getAttribute( 'desktop_direction' );
		$dd_tablet_direction = $item->getAttribute( 'tablet_direction' );
		...
		<li <?php if( $item->hasListItemClasses() ) : ?>class="<?php echo $item->getListItemClasses(); ?> <?php if ($item->hasChildren()) echo $dd_large_direction.' '.$dd_desktop_direction.' '.$dd_tablet_direction; ?>"<?php endif;?> <?php if( $item->getCssId() ) : ?>id="<?php echo $item->getCssId(); ?>"<?php endif;?>>
			...
			<a <?php if( $item->hasLinkClasses() ) : ?>class="<?php echo $item->getLinkClasses(); ?>"<?php endif;?> <?php if( $item->hasLink() ) : ?>href="<?php echo $item->getLink(); ?>"<?php endif;?> <?php if( $item->getTarget() ) : ?>target="<?php echo $item->getTarget(); ?>"<?php endif;?> <?php if( $item->hasLinkAttribs() ) : ?><?php echo $item->getLinkAttribs(); ?><?php endif;?>>
				...
				<span class="menu-item-content" data-hover="<?php echo $item->getTitle(); ?>" >
					...
					<?php if( $custom_icon && $custom_icon != -1 ) {
	...
	public function renderMenu( &$menu ) {
		...
		<div class="gf-menu-device-container responsive-type-<?php echo $gantry->get( 'responsive-menu' ); ?>">
			<?php if ($gantry->countModules('mobile-sidemenu')) : ?>
