 ...
 * @since       1.6
 */
class RokSprocketModelModule extends JModelAdmin
{
	/**
	...
	protected function preprocessForm(JForm $form, $data, $group = 'content')
	{
		...
		$lang->load($module, $client->path, null, false, false) || $lang->load($module, $client->path . '/modules/' . $module, null, false, false) || $lang->load($module, $client->path, $lang->getDefault(), false, false) || $lang->load($module, $client->path . '/modules/' . $module, $lang->getDefault(), false, false);
	...
	public function save($data)
	{
		...
		foreach ($items as $item_id => $item_settings) {
			...
			$result = $dispatcher->trigger('onExtensionBeforeSave', array(
			                                                             'com_roksprocket.module', &$item_table, true
			                                                        ));
			if (in_array(false, $result, true)) {
				$this->setError($item_table->getError());
				return false;
	...
	public function getPerItemsForm()
	{
		...
		$rcform    = new RokSprocket_ConfigForm($form);
