<?php
/**
 * @version   $Id: roksprocket.php 29278 2015-12-09 10:47:57Z jakub $
...
if (!defined('ROKSPROCKET_NEEDED_ROKCOMMON_VERSION')) define('ROKSPROCKET_NEEDED_ROKCOMMON_VERSION', '3.2.1');
...
if (!defined('ROKSPROCKET_VERSION')) define('ROKSPROCKET_VERSION', '2.1.20');
...
if (!preg_match('/project.version/', ROKCOMMON) && version_compare(preg_replace('/-SNAPSHOT/', '', ROKCOMMON), ROKSPROCKET_NEEDED_ROKCOMMON_VERSION, '<')) {
	rs_force_deactivate();
	roksprocket_set_admin_message('error', __('RokSprocket v2.1.20 needs at least RokCommon Version ' . ROKSPROCKET_NEEDED_ROKCOMMON_VERSION . '.  You currently have RokCommon Version ' . ROKCOMMON));
	return;
}
...
if (!class_exists('RokSprocket_Plugin')) {
	...
	class RokSprocket_Plugin
	{
		...
		public static function roksprocket_admin_bar()
		{
			global $wp_admin_bar;
	...
	function roksprocket_mootools_remove()
	{
		...
		if ((is_admin() && $pagenow == 'widgets.php' && !$gantry) || (is_admin() && $pagenow == 'admin-ajax.php') || (is_admin() && $page == 'roksprocket-list') || (is_admin() && $page == 'roksprocket-edit') || (!is_admin())
			...
			foreach ($wp_scripts->registered as $script) {
				if ((strpos($script->handle, 'mootools') !== false && strpos($script->handle, 'rok_') === false) || ((isset($script->content_url) && strpos($script->content_url, 'mootools') !== false) && strpos($script->handle, 'rs_') === false)) {
...
if (!function_exists('roksprocket_mootools_init')) {
	...
	function roksprocket_mootools_init()
	{
		...
		$mootools_url = RokCommon_Composite::get('rs_assets.js')->getURL('mootools.js');

		//only load mootools when needed and only in widget page if gantry is not present
		if ((is_admin() && $pagenow == 'widgets.php' && !defined('GANTRY_VERSION')) || (is_admin() && $pagenow == 'admin-ajax.php') || (is_admin() && $page == 'roksprocket-list') || (is_admin() && $page == 'roksprocket-edit') || (is_admin() && ($pagenow == 'post.php' || $pagenow == 'post-new.php')) || (is_admin() && $pagenow == 'media-upload.php') || (function_exists('is_customize_preview') && is_customize_preview())) {

			RokCommon_Header::addScript($mootools_url);
			...
			if(class_exists('Gantry\Framework\Gantry')) {
				if(!wp_script_is('mootools.js')) {
					Gantry\Framework\Document::addHeaderTag(['tag' => 'script', 'src' => $mootools_url], 'head', 10);
				}
			} else {
				RokCommon_Header::addScript($mootools_url);
			}
...
add_action('admin_enqueue_scripts', 'roksprocket_mootools_init', -50);

if (file_exists(get_template_directory() . '/templateDetails.xml')) {
	add_action('init', 'roksprocket_mootools_init', -50);
} else {
	add_action('wp_enqueue_scripts', 'roksprocket_mootools_init', -50);
}
