...
add_action( 'wp_enqueue_scripts', 'rokajaxsearch_mootools_init', -50 );
...
function rokajaxsearch_admin_css() {
	...
	<style type="text/css">
	.ras-logo {float: left; margin: 8px 10px 0 0;}
	.ras-logo #ras-icon {
		display: block;
		width: 35px;
		height: 35px;
...
function rokajaxsearch_settings_showup() {
	...
	if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true') { ?><div id="message" class="updated fade"><p><?php _e('RokAjaxSearch settings saved.', 'rokajaxsearch'); ?></p></div><?php } ?>
	
	<div class="wrap">
		<div class="ras-logo">
			<a href="http://rockettheme.com" target="_blank" id="ras-icon" title="RocketTheme LLC."></a>
		</div>
				...
				<table cellspacing="0" class="widefat fixed">
					<thead>
						<tr>
							<th></th>
							<th>
								<input type="submit" class="button button-primary rb-submit" value="<?php _e('Save Changes', 'rokajaxsearch'); ?>" style="float: right;" />
							</th>
						</tr>
					</thead>
					<tfoot>
