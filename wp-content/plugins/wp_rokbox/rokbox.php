<?php
/**
 * @version   2.50.10 December 21, 2015
...
Description: RokBox 2.50.10 is a fully responsive modal plug-in for WordPress and the successor of RokBox1. Rewritten from the ground up it can display many different media formats such as images, videos, music, embedded widgets, Ajax content and WordPress widgets and takes advantage of the new technologies such as HTML5 and CSS3.
...
class RokBox {
	...
	function _register_hooks() {
		 ...
		 * MooTools Init
		 */
		$mootools_priority = 10;

		if( is_plugin_active( 'wp_rokgallery/rokgallery.php' ) || is_plugin_active( 'wp_roksprocket/roksprocket.php' ) ) {
			$mootools_priority = -50;
		}

		add_action( 'wp_enqueue_scripts', array( &$this, 'enqueue_mootools' ), $mootools_priority);
		 ...
		 *
		 * We need to fire this on priority 11 for Gantry 4 compatibility
		 */
	...
	function compileJS() {
		...
		if( wp_script_is( 'rok_mootools_js' ) ) {
			...
			wp_enqueue_script( 'rokbox.js', $this->_url . '/assets/js/rokbox.js', array(), $this->_version, $scripts_in_footer );
		}
	}
