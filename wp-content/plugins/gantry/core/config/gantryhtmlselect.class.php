...
abstract class GantryHtmlSelect
{
	 ...
	 * <ul><li>Format options, see {@see GantryHTMLSelect::$formatOptions}.
	...
	public static function genericlist($data, $name, $attribs = null, $optKey = 'value', $optText = 'text', $selected = null, $idtag = false, $translate = false)
	{
		...
		if (isset($options['list.attr'])) {
			if (is_array($options['list.attr'])) {
				$attribs = GantryArrayHelper::toString($options['list.attr']);
			} else {
		...
		$html       = $baseIndent . '<select' . ($id !== '' ? ' id="' . $id . '"' : '') . ' name="' . $name . '"' . $attribs . '>' . $options['format.eol'] . self::options($data, $options) . $baseIndent . '</select>' . $options['format.eol'];
		return $html;
	}
	 ...
	 * <ul><li>Format options, {@see GantryHTMLSelect::$formatOptions}.
	...
	public static function integerlist($start, $end, $inc, $name, $attribs = null, $selected = null, $format = '')
	{
		...
		return GantryHTMLSelect::genericlist($data, $name, $options);
	}
	...
	{
		$options = array_merge(GantryHTMLSelect::$formatOptions, self::$_optionDefaults['option'], array(
		                                                                                                'format.depth'   => 0,
		                                                                                                'groups'         => true,
		                                                                                                'list.select'    => null,
		...
		foreach ($arr as $elementKey => &$element) {
			...
			if ($options['groups'] && $key == '<OPTGROUP>') {
				$html .= $baseIndent . '<optgroup label="' . ($options['list.translate'] ? _r($text) : $text) . '">' . $options['format.eol'];
				$baseIndent = str_repeat($options['format.indent'], ++$options['format.depth']);
				...
				if (is_array($attr)) {
					$attr = GantryArrayHelper::toString($attr);
				} else {
					$attr = trim($attr);
				}
				...
				$html .= $baseIndent . '<option value="' . ($options['option.key.toHtml'] ? htmlspecialchars($key, ENT_COMPAT, 'UTF-8') : $key) . '"' . $extra . '>' . ($options['option.text.toHtml'] ? htmlentities(html_entity_decode($text), ENT_COMPAT, 'UTF-8') : $text) . '</option>' . $options['format.eol'];
			}
		}
